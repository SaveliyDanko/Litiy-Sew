...
- Node.js + npm — для локальной сборки frontend (`npm ci`, `npm run build`).
  На VPS frontend не собирается: сервер получает только готовый `frontend/dist`,
  чтобы деплой не упирался в RAM маленького VPS.
...
Домен и email уже прописаны в [ansible/group_vars/vps/vars.yml](ansible/group_vars/vps/vars.yml):
...
https://litiy.site/media/ →  /opt/litiy-sew/uploads
```
...
| `app` | Локальная сборка backend/frontend, заливка артефактов, `docker compose up` |
...
3. **Локально** собирает frontend (`npm ci`, `npm run build`) и копирует на VPS только `frontend/dist`
4. Чинит права `/opt/litiy-sew/uploads`, чтобы Nginx мог читать загруженные фото
5. Пересобирает Docker-образ backend без кеша (runtime-only, без Gradle) и перезапускает контейнеры
...
Время выполнения: ~2–4 минуты. Сборки backend и frontend идут на dev-машине,
поэтому VPS не тратит RAM на Gradle/npm.
...
├── uploads/          # загруженные фото, отдаёт Nginx через /media/
├── docker-compose.yml
...
### Фотографии не отображаются, `/media/...` возвращает 403

Проверить один конкретный файл из API:

```bash
curl -I https://litiy.site/media/<uuid>/original.webp
```

Если ответ `403 Forbidden`, смотреть nginx error.log:
...
```text
open() "/opt/litiy-sew/uploads/<uuid>/original.webp" failed (13: Permission denied)
```

Причина: Nginx работает от `www-data`, а вложенные папки в `uploads` могут
оказаться `litiy-sew:litiy-sew` с правами `750`. Исправление уже встроено
в роль `app`, но вручную на VPS можно применить так:
...
```text
drwxr-s--- litiy-sew www-data /opt/litiy-sew/uploads/<uuid>
-rw-r----- litiy-sew www-data /opt/litiy-sew/uploads/<uuid>/original.webp
```

Если ответ `404`, URL есть в БД, но файла нет на диске. Тогда нужно восстановить
`uploads` из бэкапа или перенести медиафайлы вместе с БД.
...
Плейбук локально пересоберёт backend и frontend, загрузит готовые артефакты,
починит права `uploads` и перезапустит backend. Миграции схемы выполняет
Liquibase при старте Spring Boot.
